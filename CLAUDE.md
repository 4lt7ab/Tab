...
plugins/
  ...
  tab-for-projects/                 # "tab-for-projects" plugin package
    ...
    skills/discuss/SKILL.md         #   /discuss — agent-driven planning; runs archaeologist + project-planner + code-reviewer in parallel against a goal, then cross-questions them across rounds until forks collapse, returning one synthesized project plan with very few decisions left for the human; read-only (no MCP / code / KB writes); the user (or a future writer) commits the plan to the backlog
...
| `plugins/tab-for-projects/skills/discuss/SKILL.md` | `/discuss` — agent-driven planning; runs `archaeologist`, `project-planner`, and `code-reviewer` in parallel against a goal, then cross-questions them across rounds until forks collapse, returning one synthesized project plan with very few decisions left for the human; read-only (no MCP / code / KB writes); the user commits the plan to the backlog when ready |
