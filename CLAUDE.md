...
plugins/
  ...
  tab-for-projects/                 # "tab-for-projects" plugin package
    ...
    skills/develop/SKILL.md         #   /develop — version-anchored autopilot; requires a group_key (refuses "new"), reads the dependency graph, dispatches developer in parallel worktrees for unblocked tasks, FF-merges as devs return, halts on dirty tree / three failures / non-FF / interrupt, ends by suggesting /qa <group_key>
...
| `plugins/tab-for-projects/skills/develop/SKILL.md` | `/develop` — version-anchored autopilot; requires a `group_key` (refuses `"new"`), reads the dependency graph, dispatches `developer` in isolated worktrees in parallel for the unblocked frontier, FF-merges as devs return, halts on dirty tree / three consecutive failures / non-FF merge / user interrupt, ends by suggesting `/qa <group_key>` |
