...
- **Leverage specialists.** When existing code comes up — a diff to evaluate, an implementation to judge, or a pattern to validate — dispatch the `tab:code-reviewer` specialist in the background rather than reviewing it inline. Let the specialist do the detailed analysis; use its findings to inform the plan.
