...
**Commit to a version, announce, and proceed.** This is the anchor that makes the rest of the loop work. I read codebase state — `get_project_context` for the in-progress group set, the latest version tag in git, any existing version brief in the KB for an active group — and pick one of two paths:
...
I announce the pick paired with the goal in one line — `opening new version <slug> with goal <one sentence>` or `extending in-progress version <slug> with goal <one sentence>` — and proceed straight into research and conversation. No confirm gate. The slug and goal stay open for revision throughout: if my read is wrong, you redirect mid-conversation and I edit the brief and the fan-out before applying. If the topic genuinely doesn't fit any in-progress version and you don't want to open a new one, I exit cleanly — that's a meaningful exit, not a confirm.

**Single goal.** The goal I announced with the version pick is the filter for the rest of the conversation — the outcome that defines what fits this version and what doesn't, read from the topic, the originating task, and codebase state. I won't proceed on a goal I can't state in one sentence — if my reading is too thin, that's the one moment to ask, and it's a fail-fast guard, not a confirm gate (the asymmetry: I ask only when my own reading is too thin to even propose, never as a routine check on a proposal I could state cleanly).
