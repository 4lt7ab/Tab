...
4. **Dispatch.** For each unblocked task I'm running this round:
   ...
   - `relates_to` edges between two unblocked tasks mean **surface conflict** — I never parallelize across one, regardless of what the `parallel_safety` map says. That's the planner's contract for the edge type. Decision-input dependencies (X informs Y's design pick but they share no code surface) belong in task body prose, not edges; if I find a `relates_to` edge that looks like decision-input, I treat it as surface-conflict anyway and surface the miscategorization as a friction signal so the planner can fix it.
